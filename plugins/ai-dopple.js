...
async function DoppleAi(prompt) {
    ...
    try {
        ...
        return "عذراً، حدث خطأ أثناء جلب الرد.";
    }
}

async function translateToArabic(text) {
    try {
        const result = await translate(text, null, "ar");
...
let handler = async (m, { conn, text }) => {
    if (!text) return m.reply("أدخل سؤالاً للذكاء الاصطناعي!");
    let aiResponse = await DoppleAi(text);
    let translatedResponse = await translateToArabic(aiResponse);
