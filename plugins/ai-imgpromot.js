/*
لا تحذف Wm أيها الأخ

*مكون إضافي لتحويل الصورة إلى موجه*

نعم، فقط قل نعم على الرغم من أنك لا تعرف ماذا
...
async function imageToPrompt(buffer) {
  ...
  let { data } = await axios.post('https://www.chat-mentor.com/api/ai/image-to-text/', {
    imageUrl: image64,
    prompt: "أنشئ موجهًا نصيًا لهذه الصورة، مع التركيز على العناصر المرئية والأسلوب والميزات الرئيسية."
  }, {
    headers: {
...
let handler = async (m, { usedPrefix, command }) => {
  try {
    ...
    if (!mime || !mime.startsWith('image')) throw `قم بالرد على الصورة مع تعليق *${usedPrefix + command}* أو أرسل صورة مع تعليق *${usedPrefix + command}*`;
    ...
    if (result && result.result) {
      ...
      throw 'لا يمكن معالجة الصورة أو أن النتيجة غير صالحة.';
    }
  } catch (error) {
