...
export async function before(m) {
    let id = m.chat
    if (!m.quoted || !m.quoted.fromMe || !m.text || !/Ketik.*(calo|لمساعده)/i.test(m.quoted.text) || /.*(calo|bantuan|لمساعده)/i.test(m.text))
        return !0
    this.caklontong = this.caklontong ? this.caklontong : {}
    if (!(id in this.caklontong))
        return m.reply('انتهى هذا السؤال')
    if (m.quoted.id == this.caklontong[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            await this.reply(m.chat, `*صحيح!* +${this.caklontong[id][2]} XP\n${json.deskripsi}`, m)
